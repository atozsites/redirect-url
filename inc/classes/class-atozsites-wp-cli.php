<?php
/**
 * Setup atozsites WP CLI commands.
 *
 ...
 */
class atozsites_WP_CLI extends WP_CLI_Command {
	...
	public function cli_list() {
		...
		$redirects = atozsites_get_redirects( array( 'post_status' => 'any' ), true );
	...
	public function create( $args ) {
		...
		$ret = atozsites_create_redirect( $from, $to, $status_code, $enable_regex, $post_status );
	...
	public function import_htaccess( $args, $assoc_args ) {
		...
		foreach ( $pieces as $piece ) {
			...
			$sanitized_redirect_from = atozsites_sanitize_redirect_from( $from );
			$sanitized_redirect_to   = atozsites_sanitize_redirect_to( $to );

			$id = atozsites_create_redirect( $sanitized_redirect_from, $sanitized_redirect_to, $http_status );
			if ( is_wp_error( $id ) ) {
	...
	public function import( $args, $assoc_args ) {
		...
		foreach ( $args as $file ) {
			$processed = atozsites_import_file( $file, $mapping );
			if ( ! empty( $processed ) ) {
