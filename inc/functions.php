...
function atozsites_get_redirects( $args = array(), $hard = false ) {

	$redirects = get_transient( '_atozsites_redirects' );
	...
	if ( $hard || false === $redirects ) {
		...
		$default_max_redirects = apply_filters( 'atozsites_max_redirects', 250 );
		...
		set_transient( '_atozsites_redirects', $redirects );
	}
 ...
 * @return bool
 */
function atozsites_max_redirects_reached() {
	$default_max_redirects = apply_filters( 'atozsites_max_redirects', 250 );

	$redirects = atozsites_get_redirects();

	return ( count( $redirects ) >= $default_max_redirects );
}


function atozsites_get_valid_status_codes() {
	return apply_filters( 'atozsites_valid_status_codes', array( 301, 302, 303, 307, 403, 404 ) );
}


function atozsites_flush_cache() {
	delete_transient( '_atozsites_redirects' );
}



function atozsites_check_for_possible_redirect_loops() {
	$redirects = atozsites_get_redirects();
...
function atozsites_create_redirect( $redirect_from, $redirect_to, $status_code = 302, $enable_regex = false, $post_status = 'publish', $menu_order = 0 ) {
	global $wpdb;

	$sanitized_redirect_from = atozsites_sanitize_redirect_from( $redirect_from );
	$sanitized_redirect_to   = atozsites_sanitize_redirect_to( $redirect_to );
	...
	if ( ! in_array( $sanitized_status_code, atozsites_get_valid_status_codes(), true ) ) {
...
function atozsites_sanitize_redirect_to( $path ) {
	$path = trim( $path );
...
function atozsites_sanitize_redirect_from( $path, $allow_regex = false ) {
...
function atozsites_import_file( $file, $args ) {
	$handle       = $file;
	$close_handle = false;
	$doing_wp_cli = defined( 'WP_CLI' ) && WP_CLI;

	// filter arguments
	$args = apply_filters( 'atozsites_import_file_args', $args );
	...
	while ( ( $row = fgetcsv( $handle ) ) ) {
		...
		// sanitize
		$redirect_from = atozsites_sanitize_redirect_from( $rule[ $args['source'] ] );
		$redirect_to   = atozsites_sanitize_redirect_to( $rule[ $args['target'] ] );
		...
		// import
		$id = atozsites_create_redirect( $redirect_from, $redirect_to, $status_code, $regex, 'publish', $menu_order );
...
function atozsites_match_redirect( $path ) {
	return atozsites_Redirect::factory()->match_redirect( $path );
}
