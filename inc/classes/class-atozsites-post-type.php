...
class atozsites_Post_Type {
	...
	public function action_redirect_chain_alert() {
		...
		if ( $this->is_plugin_page() ) {
			 ...
			 */
			if ( apply_filters( 'atozsites_check_for_possible_redirect_loops', false ) ) {
				if ( atozsites_check_for_possible_redirect_loops() ) {
					?>
					<div class="updated">
						<p><?php esc_html_e( 'atozsites Redirect URL Warning: Possible redirect loops and/or chains have been created.', 'redirect-url' ); ?></p>
					</div>
					<?php
				}
			}
			if ( atozsites_max_redirects_reached() ) {
				...
				?>
				<div class="error">
					<p><?php esc_html_e( 'atozsites Redirect URL Error: You have reached the maximum allowable number of redirects', 'redirect-url' ); ?></p>
				</div>
				<?php
			}
		}
	...
	public function action_custom_redirect_columns( $column, $post_id ) {
		if ( 'atozsites_redirect_rule_to' === $column ) {
			echo esc_html( get_post_meta( $post_id, '_redirect_rule_to', true ) );
		} elseif ( 'atozsites_redirect_rule_status_code' === $column ) {
	...
	public function filter_redirect_columns( $columns ) {
		$columns['atozsites_redirect_rule_to']          = esc_html__( 'Redirect To', 'redirect-url' );
		$columns['atozsites_redirect_rule_status_code'] = esc_html__( 'HTTP Status Code', 'redirect-url' );
	...
	public function action_save_post( $post_id ) {
		...
		if ( ! empty( $_POST['atozsites_redirect_nonce'] ) && wp_verify_nonce( $_POST['atozsites_redirect_nonce'], 'atozsites-save-redirect-meta' ) && current_user_can( 'edit_post', $post_id ) ) {

			if ( ! empty( $_POST['atozsites_redirect_rule_from_regex'] ) ) {
				$allow_regex = (bool) $_POST['atozsites_redirect_rule_from_regex'];
			...
			if ( ! empty( $_POST['atozsites_redirect_rule_from'] ) ) {
				update_post_meta( $post_id, '_redirect_rule_from', atozsites_sanitize_redirect_from( $_POST['atozsites_redirect_rule_from'], $allow_regex ) );
			} else {
				delete_post_meta( $post_id, '_redirect_rule_from' );
			}

			if ( ! empty( $_POST['atozsites_redirect_rule_to'] ) ) {
				update_post_meta( $post_id, '_redirect_rule_to', atozsites_sanitize_redirect_to( $_POST['atozsites_redirect_rule_to'] ) );
			} else {
				delete_post_meta( $post_id, '_redirect_rule_to' );
			}

			if ( ! empty( $_POST['atozsites_redirect_rule_status_code'] ) ) {
				update_post_meta( $post_id, '_redirect_rule_status_code', absint( $_POST['atozsites_redirect_rule_status_code'] ) );
			} else {
				delete_post_meta( $post_id, '_redirect_rule_status_code' );
			}

			if ( ! empty( $_POST['atozsites_redirect_rule_notes'] ) ) {
				update_post_meta( $post_id, '_redirect_rule_notes', sanitize_text_field( $_POST['atozsites_redirect_rule_notes'] ) );
			} else {
	...
	protected function get_redirect_capability() {
		$redirect_capability = 'atozsites_manage_redirects';
		...
		return apply_filters( 'atozsites_restrict_to_capability', $redirect_capability );
	}
	...
	public function action_register_post_types() {
		$redirect_labels = array(
			'name'               => esc_html_x( 'atozsites Redirect URL', 'post type general name', 'redirect-url' ),
			...
			'add_new_item'       => esc_html__( 'atozsites Redirect URL', 'redirect-url' ),
			'edit_item'          => esc_html__( 'Edit Redirect Rule', 'redirect-url' ),
			'new_item'           => esc_html__( 'New Redirect Rule', 'redirect-url' ),
			'all_items'          => esc_html__( 'atozsites Redirect URL', 'redirect-url' ),
			...
			'parent_item_colon'  => '',
			'menu_name'          => esc_html__( 'atozsites Redirect URL', 'redirect-url' ),
		);
	...
	public function redirect_rule_metabox( $post ) {
		wp_nonce_field( 'atozsites-save-redirect-meta', 'atozsites_redirect_nonce' );
		...
		if ( empty( $status_code ) ) {
			$status_code = apply_filters( 'atozsites_default_direct_status', 302 );
		}
		?>
		<p>
			<label for="atozsites_redirect_rule_from"><strong><?php esc_html_e( '* Redirect From:', 'redirect-url' ); ?></strong></label><br />
			<input type="text" name="atozsites_redirect_rule_from" id="atozsites_redirect_rule_from" value="<?php echo esc_attr( $redirect_from ); ?>" />
			<input type="checkbox" name="atozsites_redirect_rule_from_regex" id="atozsites_redirect_rule_from_regex" <?php checked( true, (bool) $enable_regex ); ?> value="1" />
			<label for="atozsites_redirect_rule_from_regex"><?php esc_html_e( 'Enable Regular Expressions (advanced)', 'redirect-url' ); ?></label>
		</p>
		...
		<p>
			<label for="atozsites_redirect_rule_to"><strong><?php esc_html_e( '* Redirect To:', 'redirect-url' ); ?></strong></label><br />
			<input class="widefat" type="text" name="atozsites_redirect_rule_to" id="atozsites_redirect_rule_to" value="<?php echo esc_attr( $redirect_to ); ?>" />
		</p>
		...
		<p>
			<label for="atozsites_redirect_rule_status_code"><strong><?php esc_html_e( '* HTTP Status Code:', 'redirect-url' ); ?></strong></label>
			<select name="atozsites_redirect_rule_status_code" id="atozsites_redirect_rule_status_code">
				<?php foreach ( atozsites_get_valid_status_codes() as $code ) : ?>
		...
		<p>
			<label for="atozsites_redirect_rule_notes"><strong><?php esc_html_e( 'Notes:', 'redirect-url' ); ?></strong></label>
			<textarea name="atozsites_redirect_rule_notes" id="atozsites_redirect_rule_notes" class="widefat"><?php echo esc_attr( $redirect_notes ); ?></textarea>
