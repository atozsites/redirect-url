...
class atozsites_Redirect {
	...
	public function setup() {
		 ...
		 *   add_filter( 'atozsites_redirect_only_on_404', '__return_true' );
		 */
		if ( apply_filters( 'atozsites_redirect_only_on_404', false ) ) {
	...
	public function match_redirect( $requested_path ) {
		...
		$redirects = apply_filters( 'atozsites_registered_redirects', $redirects, $requested_path );

		// Allow for case insensitive redirects
		$case_insensitive = apply_filters( 'atozsites_case_insensitive_redirects', true );
		...
		foreach ( (array) $redirects as $redirect ) {
			...
			if ( $matched_path ) {
				...
				$sanitized_redirect_to = esc_url_raw( apply_filters( 'atozsites_redirect_to', $redirect_to ) );
	...
	public function maybe_redirect() {
		...
		if ( is_admin() || ( apply_filters( 'atozsites_redirect_only_on_404', false ) && ! is_404() ) ) {
			return;
		}

		// get requested path and add a / before it
		$requested_path = esc_url_raw( apply_filters( 'atozsites_requested_path', $_SERVER['REQUEST_URI'] ) );
		...
		do_action(
			'atozsites_do_redirect',
			$requested_path,
			$matched_redirect['redirect_to'],
			$matched_redirect['status_code']
		);

		if ( defined( 'PHPUNIT_atozsites_TESTSUITE' ) && PHPUNIT_atozsites_TESTSUITE ) {
		...
		if ( in_array( $matched_redirect['status_code'], atozsites_get_valid_status_codes(), true ) ) {
